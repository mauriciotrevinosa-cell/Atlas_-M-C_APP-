...
window.InfoModule = (() => {
  ...
  const CATEGORIES = [
    ...
    { id: 'quantum',     label: 'Quantum / MMO',    icon: '⟨ψ⟩' },
  ];
  ...
  const ITEMS = [
    ...
    /* ══════════════════════ QUANTUM / MMO THEORY ══════════════════════ */
    {
      id: 'mmo_overview',
      category: 'quantum',
      name: 'MMO — Mau\'s Market Ontology',
      icon: '⟨ψ⟩',
      desc: 'A multi-scale financial ecosystem modelled entirely through quantum and string-physics lenses. Markets are treated as quantum systems: assets have superposition states, volatility acts as a temperature field, price barriers tunnel like particles, and correlations form entanglement networks. The 4-layer architecture (GEOLOGY → SUBSURFACE → STATE → OBSERVABLE) maps structural forces all the way to what appears on the price chart.',
      source: 'Live 60-day daily OHLCV via yfinance. Fallback: deterministic synthetic GBM seeded by ticker hash.',
      how: 'Five orthogonal quantum states {BULL, BEAR, SIDEWAYS, VOLATILE, TRENDING} are treated as basis vectors. Probabilities are derived from 8 independent market features (momentum, trend strength, volatility rank, ATR, RSI, VWAP deviation, volume spike, drawdown). Each feature votes for one or more states; votes accumulate and are L1-normalised to yield the wave function |ψ⟩. All subsequent physics (tunneling, decoherence, QFI, thermal coupling) operate on this 5-component state vector.',
      tags: ['Quantum', 'MMO', 'Wave Function', '4-Layer', 'Physics', 'Markets'],
      api: '/api/mmo/quantum_state/{ticker}',
    },
    {
      id: 'mmo_born_rule',
      category: 'quantum',
      name: 'Born Rule & Wave Function Collapse',
      icon: '🌊',
      desc: 'The market wave function |ψ⟩ assigns a probability amplitude to each market state. The Born rule converts amplitudes to probabilities: P(state) = |ψ_state|². When the dominant state probability crosses a conviction threshold the wave function "collapses" — the market has picked a direction.',
      source: 'Derived from closing price returns, ATR, RSI, volume, VWAP, and momentum over the 60-day window.',
      how: 'Amplitudes are computed as: ψ_i = √(vote_i / Σvotes). Probabilities: P_i = |ψ_i|² (Born rule). Wave function is normalised so Σ P_i = 1. Collapse is triggered when max(P_i) > 0.42; the collapsed state is argmax(P_i). Shannon entropy H = −Σ P_i · log(P_i) measures superposition breadth: H ≈ 0 means strong conviction, H = log(5) ≈ 1.61 means total uncertainty (5-state equiprobability). The collapse animation in the UI represents this transition.',
      tags: ['Born Rule', 'Superposition', 'Collapse', 'Shannon Entropy', 'Probability'],
    },
    {
      id: 'mmo_cir',
      category: 'quantum',
      name: 'CIR Thermal Coupling (Cox-Ingersoll-Ross)',
      icon: '🌡',
      desc: 'Market temperature is not a metaphor — it is computed via the Cox-Ingersoll-Ross stochastic process, the same model used in interest-rate theory. A hot market scrambles wave phases and accelerates decoherence. CIR prevents temperature from going negative (just as interest rates were assumed positive in the original model).',
      source: '60-day daily returns standard deviation σ, mean reversion speed κ, long-run mean θ. All derived from yfinance history.',
      how: 'CIR update: dT = κ(θ − T)dt + σ_CIR · √T · dW. The Wiener increment is GARCH-style: dW = (r_t / σ_short) · √dt, where r_t is the latest return and σ_short is the 10-day rolling volatility. This scales the noise by how "surprising" the latest return was relative to recent volatility. The CIR constraint (√T term) automatically floors temperature at 0. β-thermal = 1 / k_B·T (k_B = 1) is the inverse temperature, representing how ordered vs. chaotic the market is. Phase correction from thermal coupling: φᵢ(T) = φ₀ᵢ + β · ω_i · T, where ω_i is the angular frequency assigned to each state layer. Hot markets (high T) scramble all phases equally; cold markets (low T) preserve phase structure.',
      tags: ['CIR', 'Temperature', 'Stochastic', 'Thermal', 'Decoherence', 'Beta'],
    },
    {
      id: 'mmo_volume_kde',
      category: 'quantum',
      name: 'Volume Profile KDE — Potential Wells V(x)',
      icon: '🏔',
      desc: 'Support and resistance levels are recast as quantum potential wells. Where volume is dense, the market is "attracted" — a particle would need energy to escape. The Volume Profile is estimated using a Gaussian kernel density estimator (KDE) weighted by trading volume, producing a smooth potential energy landscape V(x) at each price level x.',
      source: '60-day OHLCV from yfinance. Midpoint prices (H+L)/2 weighted by daily Volume.',
      how: 'scipy.stats.gaussian_kde is fit to midpoint prices with weights = daily volume / Σ volume. The KDE density ρ(x) is evaluated on a 200-point grid spanning [min_price, max_price]. Potential: V(x) = −log(ρ(x) + ε), so high-volume zones = deep wells (low V), low-volume zones = barriers (high V). Tunneling nodes are identified as the top-5 price levels where V(x) is locally maximal (barriers a particle must tunnel through). The current price locates where the market "particle" sits on this landscape.',
      tags: ['KDE', 'Volume Profile', 'Potential Well', 'scipy', 'Support', 'Resistance'],
    },
    {
      id: 'mmo_wkb',
      category: 'quantum',
      name: 'WKB Tunneling Probability',
      icon: '🚇',
      desc: 'Quantum tunneling — where a particle passes through a barrier it classically cannot surmount — models breakout probability. If the market is near a high-volume resistance barrier, WKB gives the probability that price will punch through even if momentum seems insufficient.',
      source: 'ATR (average true range) for ℏ_eff, σ_CIR for effective mass m_eff, and V(x) barriers from the KDE potential landscape.',
      how: 'WKB transmission coefficient: T = exp(−2κa), where κ = √(2 · m_eff · (V₀ − E)) / ℏ_eff. Effective Planck constant ℏ_eff = ATR / 2 (measures quantum of price motion). Effective mass m_eff = 1 / (1 + σ_CIR) — high volatility → lighter particle → easier tunneling. Barrier height V₀ = potential at the identified node, E = current market energy ≈ recent return momentum. Barrier half-width a = distance from current price to node in ATR units. Each of the top-5 nodes gets its own T_i. Tunneling Risk = "HIGH" if any T_i > 0.35, "MODERATE" if any T_i > 0.15, else "LOW".',
      tags: ['WKB', 'Tunneling', 'Breakout', 'Barrier', 'ATR', 'Mass'],
    },
    {
      id: 'mmo_qfi',
      category: 'quantum',
      name: 'Quantum Fisher Information (QFI)',
      icon: '📡',
      desc: 'Quantum Fisher Information F_Q measures how much information the market\'s current state contains about future price movement. The Cramér-Rao bound 1/√F_Q gives the minimum possible uncertainty in any unbiased estimate — a tighter bound means sharper signals. High QFI = the market is "in focus"; low QFI = the market is informationally diffuse.',
      source: 'The 5-component probability vector P = [P_BULL, P_BEAR, P_SIDEWAYS, P_VOLATILE, P_TRENDING].',
      how: 'Observable operator Â is defined as a diagonal matrix with eigenvalues aᵢ ∈ {−1, −0.5, 0, 0.5, 1} corresponding to the 5 states ordered bearish → bullish. Expectation values: ⟨Â⟩ = Σ Pᵢ · aᵢ, ⟨Â²⟩ = Σ Pᵢ · aᵢ². Variance: Var(Â) = ⟨Â²⟩ − ⟨Â⟩². Quantum Fisher Information: F_Q = 4 · Var(Â) (factor of 4 from the quantum Cramér-Rao bound for pure states). Cramér-Rao bound: σ_min = 1 / √F_Q. The QFI display in MMO\'s Heisenberg panel shows F_Q alongside σ_min to interpret signal precision.',
      tags: ['QFI', 'Fisher', 'Cramér-Rao', 'Uncertainty', 'Signal Precision', 'Information'],
    },
    {
      id: 'mmo_probability_current',
      category: 'quantum',
      name: 'Probability Current J & Directional Flow',
      icon: '🌊',
      desc: 'The probability current J tells us which direction probability mass is flowing across the 5-state space — from bearish to bullish or vice versa. Just as quantum mechanics uses J = Im(ψ* ∇ψ)/m for particle current, MMO computes a discrete J across each adjacent pair of states.',
      source: 'Wave function amplitudes ψ₀…ψ₄ computed from market features (same as Born Rule).',
      how: 'States are ordered on a bearish–bullish axis: BEAR(0) → SIDEWAYS(1) → BULL(2) → TRENDING(3) → VOLATILE(4). For each adjacent pair n, n+1: J_{n→n+1} = Im(ψₙ* · ψₙ₊₁) / ℏ_market, where ℏ_market = 0.1 (market Planck constant). The imaginary part of the cross-product captures the "phase velocity" of the wave function. Total current J_total = Σ J_n. j_directional: "BULL_FLOW" if J_total > 0.02, "BEAR_FLOW" if J_total < −0.02, else "NEUTRAL". The individual j_flows array (4 values) shows where in state-space the flow is strongest.',
      tags: ['Probability Current', 'J', 'Flow', 'Phase', 'Directional', 'Wave Function'],
    },
    {
      id: 'mmo_heisenberg',
      category: 'quantum',
      name: 'Heisenberg Uncertainty — Position Sizing',
      icon: '⚖',
      desc: 'Heisenberg\'s uncertainty principle ΔxΔp ≥ ℏ/2 is mapped to position sizing: Δx = price uncertainty (ATR-based), Δp = momentum uncertainty (Sharpe volatility). The product gives a market "uncertainty number" that scales recommended position sizes — more uncertain markets warrant smaller positions.',
      source: 'ATR-14 for price uncertainty, rolling 14-day return standard deviation × √252 for momentum uncertainty.',
      how: 'Δx = ATR_14 / close (normalised price range). Δp = σ_14 · √252 (annualised momentum volatility). Heisenberg product: ΔxΔp (market units of ℏ). Minimum product: ℏ_eff = ATR/(2·close). Uncertainty ratio r = ΔxΔp / ℏ_eff — values near 1 mean the market is at "minimum uncertainty" (strong directional conviction); r >> 1 means excess uncertainty (noisy). Position factor: pos_factor = 1 / r, so noisier markets get proportionally smaller sizing. The Heisenberg grid in the MMO HUD shows Δx, Δp, ℏ_eff, and pos_factor together.',
      tags: ['Heisenberg', 'Uncertainty', 'Position Sizing', 'ATR', 'Volatility', 'Risk'],
    },
    {
      id: 'mmo_decoherence',
      category: 'quantum',
      name: 'Decoherence Time τ',
      icon: '⏳',
      desc: 'In quantum mechanics, decoherence is the process by which a quantum superposition collapses into a classical state due to environmental noise. In MMO, τ is the estimated time (in bars) before the market\'s current superposition becomes untrustworthy — driven by volatility (σ_CIR) and Shannon entropy (H). High noise + high entropy = fast decoherence = signals expire quickly.',
      source: 'σ_CIR (CIR temperature term) and Shannon entropy H from the 5-state probability vector.',
      how: 'Decoherence formula: τ = 1 / (σ_CIR × H_entropy), measured in trading days. σ_CIR is the annualised CIR volatility parameter (≈ 60-day return σ). H_entropy is the Shannon entropy of the probability vector (0 = fully collapsed, 1.61 = maximum uncertainty). Floor at τ = 0.5 bars. Interpretation: τ < 1 means "act now, signal expires within one bar"; τ > 5 means "stable regime, signal is reliable for multiple sessions". The decoherence clock in the MMO dashboard ticks down τ in real time.',
      tags: ['Decoherence', 'Tau', 'Entropy', 'Signal Lifetime', 'Noise', 'CIR'],
    },
    {
      id: 'mmo_4layer',
      category: 'quantum',
      name: '4-Layer Market Ecosystem',
      icon: '🌍',
      desc: 'MMO organises all market forces into four nested layers, mirroring geological depth: GEOLOGY (structural fundamentals) → SUBSURFACE (energy and capital flows) → STATE (thermal quantum regime) → OBSERVABLE (what appears on the chart). Each layer has its own physics and feeds into the one above it.',
      source: 'Synthesised from all computed indicators: structure scores from fundamentals, energy from momentum and volume, thermal from CIR, observable from OHLCV.',
      how: 'GEOLOGY layer — structural metrics: liquidity score (bid-ask spread proxy), market cap stability, sector beta, long-run mean reversion κ from CIR. SUBSURFACE layer — energy metrics: kinetic energy E_k = ½ · σ² (volatility as KE), potential energy E_p from V(x) KDE, bubble risk = E_k / E_p, capital fatigue = rolling drawdown depth, cooling rate = dT/dt from CIR. STATE layer — thermal metrics: T (market temperature), β (inverse temperature), CIR drift dr/dt, decoherence τ. OBSERVABLE layer — quantum observables: |ψ⟩ wave function, Born probabilities, collapsed state, J_total, QFI, Heisenberg product.',
      tags: ['4-Layer', 'Geology', 'Energy', 'Thermal', 'Observable', 'Architecture'],
    },
    {
      id: 'mmo_string',
      category: 'quantum',
      name: 'String Theory Vibration Layer',
      icon: '🎵',
      desc: 'String Theory provides the deepest layer of MMO: every asset is treated as a vibrating string in 10-dimensional price-space. The string\'s amplitude (volatility envelope), frequency (cycle speed), vertices (turning points in 30 days), and nodes (zero-crossing periods of calm) characterise the market\'s "music". Entanglement between strings describes cross-asset correlation.',
      source: '60-day OHLCV. Vertices: local extrema of the close price. Nodes: periods where |return| < 0.001.',
      how: 'String amplitude A = std(returns) × √252 (annualised volatility envelope). String frequency f = vertices_30d / 30 (turning points per day). String tension τ_s = A / (f + ε) — low tension = slow, calm swings; high tension = rapid, violent vibrations. Vertices_30d counted by identifying local price peaks and troughs (numpy.argrelextrema, order=2). Nodes_30d = days with |return| < 0.001 (flat vibration). Entanglement score = cross-ticker correlation of the 5-state probability vectors, shown as a network in the MMO scan panel.',
      tags: ['String Theory', 'Vibration', 'Amplitude', 'Frequency', 'Vertices', 'Entanglement'],
    },
    {
      id: 'mmo_phase3b',
      category: 'quantum',
  ...
  let _category = 'all';
  let _items = ITEMS;
  let _sourceMode = 'local fallback';
  let _loading = false;
  let _loadTimer = null;
  ...
  async function _loadCatalog({ includeWeb = false } = {}) {
    ...
    try {
      ...
      _items = ITEMS;
      _sourceMode = `local fallback: ${error.message}`;
    } finally {
      _loading = false;
      _render();
    }
